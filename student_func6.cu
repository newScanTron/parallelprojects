...
__global__ void getMask(const uchar4* d_sourceImg)
{
                ...
                uchar4* const h_blendedImg) //OUT
{
  ...
  /* To Recap here are the steps you need to implement
     ...
     1) Compute a mask of the pixels from the source image to be copied
        ...
        const size_t source_size = sizeof(const uchar4 const) * numRowsSource * numColsSource;
        const uchar4* d_sourceImg;
        checkCudaErrors(cudaMalloc(&d_sourceImg, source_size));
        checkCudaErrors(cudaMemcpy(&d_sourceImg, h_sourceImg, source_size, cudaMemcpyHostToDevice));
